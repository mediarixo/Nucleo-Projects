...
#define _DRV_RCC_GPIOA_CLK_ENABLE()       (RCC->AHB1ENR |=  (1 << 0) )
#define _DRV_RCC_GPIOB_CLK_ENABLE()       (RCC->AHB1ENR |=  (1 << 1) )

#define _DRV_RCC_SPI1_CLK_ENABLE()        (RCC->APB2ENR |=  (1 << 12) )
...
void gpio_init(void) {
	...
	GPIOB->ODR |=  (1 << SPI_CS_PIN);		//pull to HIGH
}

void spi_init(void) {
	_DRV_RCC_SPI1_CLK_ENABLE();
